...
pot2 = pot_data.iloc[9:16, :]
pot3 = pot_data.iloc[17:24, :]
pot4 = pot_data.iloc[25:32, :]
...
groupA = potbig.iloc[[0, 7, 15, 23], :]
groupA = groupA.set_index([pd.Index([0, 1, 2, 3]), ])
print(groupA)

for i, j in zip((0, 0, 0, 1, 1, 2), (1, 2, 3, 2, 3, 3)):
    if i < j:
        team1 = groupA.loc[i, 'Country']
        team2 = groupA.loc[j, 'Country']
        a1, d1 = groupA.loc[i, 'Attack'], groupA.loc[i, 'Defence']
        a2, d2 = groupA.loc[j, 'Attack'], groupA.loc[j, 'Defence']
        p1 = 0.014 * (a1 / d2)
        q1 = 1 - p1
        p2 = 0.014 * (a2 / d1)
        q2 = 1 - p2
